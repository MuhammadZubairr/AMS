...
async function getAllEmployees({ limit = 10, offset = 0, search = null } = {}) {
  ...
  const countResult = await db.query(countQuery, params.slice(0, params.length - (search ? 0 : 0)));
...
async function createEmployee(email, passwordHash, name) {
  ...
  const { rows } = await db.query(query, [email, passwordHash, name]);
  return rows[0];
}
...
async function getEmployeeById(id) {
  ...
  const { rows } = await db.query(query, [id, 'employee']);
  return rows[0] || null;
}
...
async function updateEmployee(id, { name, email }) {
  ...
  const { rows } = await db.query(query, [id, name || null, email || null]);
  return rows[0] || null;
}
