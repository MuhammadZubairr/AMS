...
export default function HRSidebar({ 
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 z-40 h-screen transform border-r border-slate-200 bg-white text-slate-800 shadow-sm transition-[transform,width,box-shadow] duration-300 ease-in-out ${
        ...
        <div className="flex flex-col h-full">
          <div className="flex h-16 items-center justify-between border-b border-slate-100 px-4 md:px-5">
            <div className="flex items-center gap-3 overflow-hidden">
              {collapsed ? <BrandLogo compact /> : <BrandLogo subtitle="Human Resources" />}
            </div>
            {/* Desktop Collapse Toggle */}
            <button
              type="button"
              className="hidden md:flex rounded-xl p-2 text-slate-500 transition hover:bg-slate-100"
              onClick={onCollapseToggle}
              aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}
            >
              <SuperAdminIcon name="collapse" className={`h-5 w-5 transition-transform duration-300 ${collapsed ? 'rotate-180' : ''}`} />
            </button>
            {/* Mobile Close Button */}
            <button
              type="button"
          ...
          <nav className="flex-1 min-h-0 space-y-1 p-3 pr-2 overflow-y-auto">
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  {!collapsed && isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-10 w-1 rounded-r-full bg-[#2563EB]" />}
          ...
          <div className="mt-auto border-t border-slate-100 p-4">
            ...
            >
              ...
              {!collapsed && <span>{isLogoutLoading ? 'Logging out...' : 'Logout'}</span>}
            </button>
          </div>
        </div>
      </aside>
